...
const navigation = [
  { name: 'Browse Homes', href: '/homes' },
  { name: 'Contact Us', href: '/contactus' },
  { name: 'Dashboard', href: '/myhomes' },
  { name: 'Host Home', href: '/homes/host' },
    ...
    <header className={'fixed inset-x-0 top-0 w-full z-50 bg-gray-100 '}>
      <nav aria-label="Global" className="flex items-center justify-between   p-5 lg:px-8 ">
        ...
        <div className="hidden lg:flex lg:gap-x-12" >
          {navigation.map((item) => (
            <a key={item.name} href={item.href} className="text-sm text-pretty font-medium leading-6  text-gray-900">
              {item.name}
            </a>
          ))}
        </div>
        ...
        <div className="hidden lg:flex lg:flex-1 lg:justify-end">
          ...
          <SignedOut>
            <Link href='/sign-in' className="text-sm text-pretty rounded-md  px-2.5 py-1.5 font-semibold text-gray-900  ">
              sign in
            </Link>
          </SignedOut>
        </div>
